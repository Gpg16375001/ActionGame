...
//	BBcheck( ) ;
	...
	*/
	...
	if ( (pp->pchg[0] <= 0) )							// É^ÉCÉ}Å[ÇÃÉJÉEÉìÉgÇ™ 0à»â∫Ç…Ç»Ç¡ÇΩÇÁ
	{
...
void AeIJump( void )
{
	pp->yspd = -15.0 ;									// è„ï˚å¸Ç…èâë¨Çí«â¡
...
void EnemySet( void )
{
	...
	for ( y = 0 ; y < 18 ; y++ )
	{
		for ( x = 0 ; x < 25 ; x++ )
		{
			...
			{
				obj[no].idno = ID_ENE ;					// ÇªÇÃèÍèäÇ…ÉGÉlÉ~Å[Çì¸ÇÍÇÈ
				obj[no].mode = 0 ;						// MODE Çèâä˙ÉZÉbÉgÇ…Ç∑ÇÈ
					
				obj[no].xp = x * 32 ;					// X ç¿ïWÇçáÇÌÇπÇÈ
				obj[no].yp = y * 32 + 32 ;				// Y ç¿ïWÇçáÇÌÇπÇÈ
			}
		}
	}
